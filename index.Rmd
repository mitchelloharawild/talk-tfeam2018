...
fbl_elec_fit <- tsibbledata::elecdemand %>% 
  filter(index < ymd("2014-03-01")) %>%
  fable::ARIMA(Demand ~ pdq(1,1,1) + PDQ(1,0,1, period = "day") + 
...
ts_elecdemand <- ts(head(fpp2::elecdemand, 2832), frequency = 48)
fc_elec_fit <- ts_elecdemand[,"Demand"] %>% 
  forecast::Arima(order = c(1,1,1), seasonal = c(1,0,1),
                  xreg = cbind(ts_elecdemand[,c("WorkDay", "Temperature")], 
                               ts_elecdemand[,"Temperature"]^2))
```
...
tsbl_elec_new <- tsibbledata::elecdemand %>% 
  filter(year(index) == 2014, month(index) == 3)
...
ts_elec_new <- ts(fpp2::elecdemand[2833:4320,], frequency = 48)
